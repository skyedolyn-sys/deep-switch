...
| macOS (Intel) | 소스에서 빌드 | — |
| Linux / Windows | 소스에서 빌드 | — |

> ⚠️ **빌드는 서명도, 공증도 되지 않았습니다.** 첫 실행 시 macOS가 *"Deep Switch가 손상되어 열 수 없습니다"* 라고 표시합니다. 우회 방법:
> 1. DMG를 열고 **Deep Switch**를 `/Applications`로 드래그합니다.
> 2. **Finder**에서 `/Applications`로 이동하여 **Deep Switch**를 우클릭 → **열기** → 대화 상자에서 확인.
> 3. 이후에는 더블 클릭으로 정상 실행됩니다.
>
> 터미널 방법: `sudo xattr -dr com.apple.quarantine "/Applications/Deep Switch.app"`
>
> Apple Developer ID를 확보한 후 서명 + 공증 버전으로 전환할 예정입니다. 그때까지 위의 우회 방법을 사용해 주세요.
...
- 🧩 **공급자가 너무 많고, 키도 너무 많습니다** — DeepSeek, Moonshot, Zhipu GLM, MiniMax, ByteDance Doubao, SiliconFlow, OpenRouter, OpenAI, Groq… 각각 고유한 Base URL, 모델 이름, 함정이 있습니다.
...
- 📦 **사전 설정 폭넓은 커버리지** — DeepSeek · Moonshot/Kimi · Zhipu GLM · MiniMax · ByteDance Doubao · SiliconFlow · OpenRouter · OpenAI · Groq, 그리고 완전히 사용자 정의 가능한 *Custom* 슬롯.
...
- 📌 **트레이 메뉴로 즉시 전환** — 트레이 아이콘을 우클릭하여 메인 창을 열지 않고 전환. 메뉴 표기는 UI 언어에 따름.
- 🪄 **최초 "현재 설정 감지" 가져오기** — Deep Code가 현재 가리키는 것을 읽어 한 번의 클릭으로 저장 가능한 공급자로 만듦.
- 🔒 **로컬 전용 자격 증명** — `~/.deep-switch/config.json`에 저장. 업로드, 동기화, 로그 저장 모두 없음.
...
[Releases](../../releases) 페이지에서 최신 `.dmg`를 다운로드하고 **Deep Switch**를 Applications 폴더로 드래그합니다.

> 빌드가 서명되지 않았으므로 DMG에서 처음 실행할 때 Gatekeeper 경고가 나타납니다. 우클릭 → *열기*로 허용하세요.

### Linux 및 Windows

공식 설치 프로그램은 아직 없지만, 소스에서 실행할 수 있습니다:
...
| 영속화          | `~/.deep-switch/config.json` + `~/.deepcode/settings.json` |
...
API를 제공해 주는 공급자들에게 감사드립니다 — **DeepSeek, Moonshot / Kimi, Zhipu GLM, MiniMax, ByteDance Doubao, SiliconFlow, OpenRouter, OpenAI, Groq** — 그리고 Tauri, React, Vite, Rust, i18next의 메인테이너들께. 여러분의 노력 덕분에 이 앱은 쉽게 만들어질 수 있습니다.
